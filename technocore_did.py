...
# 仕様: C0/C1制御文字（改行含む）、フォーマット文字、ZWJ、bidiオーバーライドを
#       すべてスペースに置換。Unicodeカテゴリ Cc / Cf が該当する。
#
# Zl（U+2028 LINE SEPARATOR）と Zp（U+2029 PARAGRAPH SEPARATOR）も対象に含める。
# これらは Cc/Cf ではないが「行区切り」であり、single-line sweep を名乗る以上
# サーバ側も潰している可能性が高い。仮にサーバが潰していなくても、こちらが
# 先に潰しておけば sweep 済みテキストはサーバ sweep の不動点のままなので、
# 過剰に sweep する側に倒すのが安全。逆（取りこぼし）は署名検証の失敗になる。
# --------------------------------------------------------------------------
SWEEP_CATEGORIES = ("Cc", "Cf", "Zl", "Zp")
...
def verify_locally(did: str, room: str, nonce: int, swept: str, sig_b64: str) -> bool:
    ...
    try:
        pub.verify(sig, f"{room}|{nonce}|{swept}".encode("utf-8"))
        return True
    except Exception:
        return False
